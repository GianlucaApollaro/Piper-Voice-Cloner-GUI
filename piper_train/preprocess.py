...
def main() -> None:
    print("**************************************************")
    print("   STARTING LOCAL MODIFIED PREPROCESS SCRIPT      ")
    print("**************************************************")
    ...
    # STRATEGY: Write to temp, then copy.
    # This avoids open file handles on the mounted volume during multiprocessing.
    temp_dataset_path = Path("/tmp/dataset.jsonl")
    final_dataset_path = args.output_dir / "dataset.jsonl"
    
    print(f"DEBUG: Writing to temporary file: {temp_dataset_path}", flush=True)

    with open(temp_dataset_path, "w", encoding="utf-8") as dataset_file:
    ...
    _LOGGER.debug("Signaling workers to stop")
    for proc in processes:
        queue_in.put(None)

    # Wait for workers to stop
    _LOGGER.debug("Waiting for workers to join")
    for proc in processes:
        proc.join(timeout=1)
    ...
    # We use dataset_safe.jsonl because the standard dataset.jsonl filename 
    # encounters persistent 'File exists' errors on this specific Windows/Docker mount setup.
    safe_target_path = args.output_dir / "dataset_safe.jsonl"
    print(f"DEBUG: Copying {temp_dataset_path} to SAFE path: {safe_target_path}", flush=True)
    
    try:
        shutil.copy2(temp_dataset_path, safe_target_path)
        print("DEBUG: Copy to dataset_safe.jsonl SUCCESSFUL!", flush=True)
    except Exception as e:
        print(f"FATAL ERROR COPYING FILE: {e}", flush=True)
        # Fallback to system cp
        ret = os.system(f"cp {temp_dataset_path} {safe_target_path}")
        if ret != 0:
             print("FATAL: System cp also failed. Exiting with error.", flush=True)
             sys.exit(1)

    print("--- DEBUG: PREPROCESS MAIN FINISHED SUCCESSFULLY ---", flush=True)
    sys.exit(0)
        ...
        while True:
            ...
            for utt in utt_batch:
                try:
                    ...
                    # Robust Catch-All for processing
                    _LOGGER.debug(utt)
                    ...
                    queue_out.put(None) # Sync main thread
                except Exception as e:
                    _LOGGER.error(f"SKIPPING CORRUPTED FILE {utt.audio_path}: {e}")
                    queue_out.put(None) # Sync main thread
